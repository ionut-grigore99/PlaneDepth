...
# Move up two levels to reach the root of the project (planedepth)
project_root = os.path.dirname(current_script_dir)
splits_dir = os.path.join(project_root, "splits")


# Models which were trained with stereo supervision were trained with a nominal
# baseline of 0.1 units. The KITTI rig (rig = an apparatus, device, or piece of equipment
# designed for a particular purpose) has a baseline of 54cm. Therefore,
...
def evaluate():
    ...
    if path_disp_to_eval is None:

        load_weights_folder = os.path.expanduser(get('load_weights_folder'))
        ...
        if get('model').get('net_type') == "ResNet":
            ...
            encoder = networks.ResnetEncoder(get('model').get('num_resnet_layers'), False)
            ...
            model = networks.PladeNet(False, 
                                      ...
                                      render_probability=get('render_probability'),
                                      plane_residual=get('plane_residual'))
            model.load_state_dict(torch.load(os.path.join(load_weights_folder, "plade.pth")))
            if get('use_cuda') is True:
                model.cuda()
            model.eval()
        elif get('model').get('net_type') == "FalNet":
            model = networks.FalNet(False, get('im_sz')[0], get('im_sz')[1], get('disp_levels'), get('disp_min'), get('disp_max'))
            model.load_state_dict(torch.load(os.path.join(load_weights_folder, "fal.pth")))
            if get('use_cuda') is True:
                model.cuda()
            model.eval()
        ...
        grid = torch.meshgrid(torch.linspace(-1, 1, get('im_sz')[0]), torch.linspace(-1, 1, get('im_sz')[1]), indexing="xy")
        ...
        print("-> Loading predictions from {}".format(path_disp_to_eval))
        pred_disps = np.load(path_disp_to_eval)
    ...
    if save_pred_disps:
        output_path = os.path.join(
            load_weights_folder, "disps_{}_split.npy".format(eval_split))
...
if __name__ == "__main__":
    ...
    eval_stereo = True  # if set evaluates in stereo mode
    eval_mono = False  # if set evaluates in mono mode
    ...
    path_disp_to_eval = None # optional path to a .npy disparities file to evaluate
