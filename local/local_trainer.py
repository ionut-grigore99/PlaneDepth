...
class Trainer:
    def __init__(self, options):
        ...
        if len(self.opt.novel_frame_ids) > 0 and not self.opt.use_colmap:
            ...
            self.models["pose"] = networks.PoseDecoder(self.models["pose_encoder"].num_ch_enc, num_input_features=1,
                                                       num_frames_to_predict_for=1, num_ep=8)

        for model_name, model in self.models.items():
            model = model.to(self.device)
            model = nn.SyncBatchNorm.convert_sync_batchnorm(model)
            self.models[model_name] = torch.nn.parallel.DistributedDataParallel(model, device_ids=[self.local_rank],
        ...
        self.num_total_steps = num_train_samples // (
                    self.opt.batch_size * torch.cuda.device_count()) * self.opt.num_epochs
        ...
        train_dataset = self.dataset(
            ...
            self.opt.novel_frame_ids, is_train=True, use_crop=not self.opt.no_crop, use_colmap=self.opt.use_colmap,
            colmap_path=self.opt.colmap_path, img_ext=img_ext)
        self.train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, False,
            num_workers=self.opt.num_workers, sampler=self.train_sampler, pin_memory=True, drop_last=True,
    ...
    def create_models(self):
        ...
        if self.opt.net_type == "ResNet":
            ...
            print("train FalNet")
            self.models["fal"] = networks.FalNet(False, self.opt.height, self.opt.width, self.opt.disp_levels,
                                                 self.opt.disp_min, self.opt.disp_max)
    ...
    def add_flip_right_inputs(self, inputs):
        ...
        new_inputs[("color_aug", "l")] = torch.cat([inputs[("color_aug", "l")], inputs[("color_aug", "r")].flip(-1)],
                                                   dim=0)
        new_inputs[("color_aug", "r")] = torch.cat([inputs[("color_aug", "r")], inputs[("color_aug", "l")].flip(-1)],
                                                   dim=0)
        grid_fliped = inputs["grid"].clone()
        grid_fliped[:, 0, :, :] *= -1.
        grid_fliped = grid_fliped.flip(-1)
        new_inputs["grid"] = torch.cat([inputs["grid"], grid_fliped], dim=0)
        new_inputs[("depth_gt", "l")] = torch.cat([inputs[("depth_gt", "l")], inputs[("depth_gt", "r")].flip(-1)],
                                                  dim=0)
        new_inputs[("depth_gt", "r")] = torch.cat([inputs[("depth_gt", "r")], inputs[("depth_gt", "l")].flip(-1)],
                                                  dim=0)
    ...
    def pred_novel_images(self, inputs, outputs):
        ...
        for target_side in self.target_sides:
            if self.opt.warp_type == "depth_warp":
                ...
                cam_points = self.backproject_depth(depths.reshape(B * N, 1, H, W),
                                                    inputs["inv_K"][:, None, :, :].expand(-1, N, -1, -1).reshape(B * N,
                                                                                                                 4, 4))
                pix_coords = self.project_3d(cam_points,
                                             inputs["K"][:, None, :, :].expand(-1, N, -1, -1).reshape(B * N, 4, 4),
                                             T)  # BN, H, W, 2
    ...
    def load_model(self):
        ...
        for n in self.opt.models_to_load:
            ...
            pathfrom __future__ import absolute_import, division, print_function
import copy
import random
...
if __name__ == '__main__':
    ...
    trainer.train() = os.path.join(self.opt.load_weights_folder, "{}.pth".format(n))
            model_dict = self.models[n].module.state_dict()
            pretrained_dict = torch.load(path, map_location=self.device)
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
            model_dict.update(pretrained_dict)
