...
if __name__=='__main__':
    ...
    option = 1  # 1 means PoseEncoder, 2 means PoseDecoder

    if option == 1:
        # for pose_encoder
        model=ResnetPoseEncoder(num_layers=get('model').get('num_resnet_layers'), pretrained=True, num_input_images=2)
        ...
        torch.onnx.export(model,  # model being run
                          ...
                          "resnet_pose_encoder.onnx",  # where to save the model (can be a file or file-like object)
        ...
        #for pose_decoder
        model=PoseDecoder(num_layers=get('model').get('num_resnet_layers'), pretrained=True)
        architecture = summary(model, torch.rand(1, 3, *get('im_sz')), max_depth=4, show_parent_layers=True,
                               print_summary=True)
